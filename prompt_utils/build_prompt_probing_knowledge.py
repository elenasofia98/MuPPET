...
def build_system_prompt(
    ...
    system_prompt = process_markdown(
            input_path=f'./prompts/probing-prompts/system-{exp_setup}.md', #if defense_strategy is None else  f'./prompts/defense-prompts/system-{defense_strategy}.md',
...
def build_user_prompt(
    ...
    if target_user is None and exp_setup == 'knowledge-single':
        print("*"*80)
        print("Target user is None but the exp setup is ", exp_setup)
        print("*"*80)
    ...
    variables['candidate_information'] = candidate_information

    if target_user is not None:
        variables['target_user'] = target_user

    message_prompt = process_markdown(
            input_path=f'./prompts/probing-prompts/user-{exp_setup}.md', #if defense_strategy is None else f'./prompts/defense-prompts/user_message-{defense_strategy}.md',
            variables=variables,
            use_env=False,
            fail_on_missing=True
        )
